...
class IjcsclParser(Parser):
    ...
    def check_limits(self, line: LTTextLineHorizontal, page_index: int = 0, column: int = 0) -> bool:
        if page_index == 0:  # we are on first page
            if line.y1 < 500:  # before abstract
                return True
        else:
            if line.y1 < 600:
                return True
    ...
    def check_heading(self, line: LTTextLineHorizontal, heading_list: List[Heading] = None) -> bool:
        ...
        if potential_subheading:
            heading = self.check_heading_list(line, heading_list)
            if heading:
                return True
        else:
            ...
            if potential_subheading:
                if "." in line.get_text() or 80 < line.x0 < 90:
                    return True
                if len(fonts.most_common()) == 1:
                    return True
            return False
